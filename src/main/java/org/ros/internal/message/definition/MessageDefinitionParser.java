...
public class MessageDefinitionParser {
  ...
  private void parseField(String messageType, String fieldDefinition) {
    ...
    if (name.contains("=") && (!name.contains("#") || name.indexOf('#') > name.indexOf('='))) {
      ...
      assert(!name.startsWith("#")) : String.format("Fields must define a name. Field definition in %s was: \"%s\"", messageType, fieldDefinition);
    ...
    if (type.equals("Header")) {
      ...
      assert(name.equals("header")) : "Header field must be named \"header.\"";
    ...
    if (value != null) {
      ...
      if (!type.equals(PrimitiveFieldType.STRING.getName()) && value.contains("#")) {
        assert(!value.startsWith("#")) : "Constants must define a value.";
